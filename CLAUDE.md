...
GitHub Actions (`.github/workflows/cd.yml`) runs `yarn generate` and deploys `dist/` to GitHub Pages on push to `main`, then deploys the database rules and Cloud Functions (`firebase deploy --only database` / `--only functions`) authenticated with the `FIREBASE_SERVICE_ACCOUNT_3AE94` key (the `github-action-…` SA). Firebase credentials are injected as GitHub secrets. The `ci.yml` workflow runs `yarn lint`, `yarn test`, and `yarn workspace functions run build` on push to `main` and on PRs. A separate `rules.yml` workflow runs `yarn test:rules` (RTDB emulator + `actions/setup-java`, Temurin 21) — gated by a native `paths:` filter so it only runs when `database.rules.json`, `test/rules/**`, `vitest.rules.config.ts`, or `firebase.json` changed (these tests need Java and an emulator, so they're kept off the hot path). Keep it as a separate workflow, not a path-filtered job inside `ci.yml`: GitHub's built-in `paths:` filter only works at the workflow-trigger level, and a path-filtered workflow must not be made a required status check (a skipped run never reports).
